...
// Polyfill of PriorityQueue, available starting .NET 6 :'(
...
namespace System.Collections.Generic
{
  ...
  {
    private delegate bool IsReferenceOrContainsReferencesDelegate<T>();

    private static IsReferenceOrContainsReferencesDelegate<(TElement, TPriority)> IsReferenceOrContainsReferences;
    ...
    static PriorityQueue()
    {
      ...
      IsReferenceOrContainsReferences = (IsReferenceOrContainsReferencesDelegate<(TElement, TPriority)>)
        Type.GetType("System.Runtime.CompilerServices.RuntimeHelpers")
          ...
          .MakeGenericMethod(typeof((TElement, TPriority)))
          .CreateDelegate(typeof(IsReferenceOrContainsReferencesDelegate<(TElement, TPriority)>));
    }
    ...
    public void EnqueueRange(IEnumerable<(TElement Element, TPriority Priority)> items)
    {
      ...
      if (null != collection && (count = collection.Count) > _nodes.Length - _size)
      {
        Grow(_size + count);
      }
    ...
    private void Grow(int minCapacity)
    {
      ...
      if ((uint)newcapacity > ArrayPolyfill.MaxLength) newcapacity = ArrayPolyfill.MaxLength;

      // Ensure minimum growth is respected.
      newcapacity = Math.Max(newcapacity, _nodes.Length + MinimumGrow);

      // If the computed capacity is still less than specified, set to the original argument.
      // Capacities exceeding ArrayPolyfill.MaxLength will be surfaced as OutOfMemoryException by Array.Resize.
  ...
  internal static class SR
  {
    ...
    internal static string ArgumentOutOfRange_IndexMustBeLessOrEqual => "Index was out of range. Must be non-negative and less than or equal to the size of the collection.";
    ...
    internal static string InvalidOperation_EnumFailedVersion => @"Collection was modified after the enumerator was instantiated.";
  }
}
